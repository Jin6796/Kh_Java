...
public class Board3Logic {
	...
	public int boardInsert(Map<String, Object> pMap) {
		...
		// board_master_t에 입력 성공 시 담기
		int result = 0;
		// board_sub_t에 입력 성공 시 담기
		int result2 = 0;
		int b_no = 0;
		int b_group = 0;
		// 글번호 채번할 때 한 번--------------------------------------------------
...
//			아래 코드는 내 뒤에 댓글이 있을 때에만 처리가 됨
		...
		result = boardMDao.boardMInsert(pMap); // 새 글 쓰기, 댓글 쓰기, 
		...
		// 부모에서 자식으로 내리는 건 가능하지만 반대는 불가능 (리액트 > 리덕스 > NextJS)
		// 전혀 문제가 되지 X - static으로 해결 - POJO의 한계 >> SPRING, SPRING BOOT
		/*
