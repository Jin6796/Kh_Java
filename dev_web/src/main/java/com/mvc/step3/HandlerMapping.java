...
//	new ModelAndView();
	...
	public static Object getController(String[] upmu, HttpServletRequest req, HttpServletResponse res) {
		...
		if("board3".equals(upmu[0])) { // 배열의 첫방에 업무폴더 이름
			controller = new Board3Controller();
			// 게시판 글쓰기 메소드 호출
			// if문 사용하여 메소드 이름을 결정지었다.
			// if문이 늘어가서 코드의 양도 같이 늘어남.....
			...
			// 문제 제기 >> NullPointerException이 발생하잖아 > spring이 대신 할게 > 의존성 주입(Dependency Injection)
			// 외부(스프링)에서 필요할 때 주입[객체를 메모리에 로딩]해준다 (게을러 게을러~)
			// 스프링이 어떤 클래스인지는 어떻게 아는겨??? >> xml문서에 미리 등록해놓음.
			// setter 객체 주입법과 생성자 객체 주입법
