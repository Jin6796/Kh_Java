...
// void > ActionForward > String(spring 4.0, 5.0, boot) > ModelAndView(Object, scope 처리: 스프링 )
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import org.apache.log4j.Logger;
// 시중 교재에서는 어노테이션으로 url 매핑을 처리하지만
// 수업에서는 스프링 프레임워크를 최대한 흉내내보자는 취지로 사용하지 않는다.
...
public class ActionServlet extends HttpServlet {
	...
	public void doService(HttpServletRequest req, HttpServletResponse res) 
		...
		// 컨텍스트는 톱캣 서버의 server.xml에 배포된 위치 정보 값을 참조
		// /dev_web or /
		...
		// 컨트롤 클래스에서 리턴타입을 String, ModelAndView(Spring2.5) 이원화 해보기
		Object obj = null;
		obj = boardController.execute(req, res);
		// page로 넘어오는 값의 유형
		// redirect: board2/boardInsert -> :이 있는 경우
...
//					pageMove[0]= redirect of forward - redirect 유무 담아주세요
//					pageMove[1]= board2/boardList - 페이지 이름을 담아주세요
			...
			// 찾아서 연결해주고 각 업무별 추가된 컨트롤 클래스에서 지정한 페이지에 대한 요청 URL의 조립은
			...
			if(pageMove !=null) {
				...
				//Board2Controller에 리턴 값이 return "forward:boardList.jsp"일 때
