...
//컴넌트들의 위치를 정해줍니다.
	 ...
	 * @param editable 이론 인스턴스화 후 입력 컴포넌트에 쓰기를 허용할건가? 아님 읽기전용으로 할 건가를
	 * 		다이얼로그.setVisible(false) ->setVisable(true)
	 * @param vo
	 * @param abook
	 */
	// 자바 리펙토링
	...
	private void setEditable(boolean e) {
		txtName.setEditable(e); // true가 오면 이름을 변경, false가 오면? 변경 불가!
	...
	// 콜백함수는 원본을 건드리지 않는다
  	private void btnOkayActionPerformed(ActionEvent evt) {
  		if(getName().trim().length() == 0) {
  			JOptionPane.showMessageDialog(this, "이름을 입력하세요.", "Error", JOptionPane.ERROR_MESSAGE);
  			return;
  		}
		isCancel= false;
		
		//수정화면에서 확인버튼을 눌렀을 때와 입력화면에서 확인 버튼을 눌렀을 때와 
		//입력화면에서 확인 버튼을 눌렀을 때 처리하기
		//아이디가 존재하면 수정 모드, 그렇지 않으면 입력 모드로 처리한다.
		if(avo != null){
			JOptionPane.showMessageDialog(this, "수정하기 확인 완료","INFO", JOptionPane.INFORMATION_MESSAGE);		
			try{
				AddressVO vo = new AddressVO();	
				// vo를 send 메소드의 파라미터로 넘기는 이유가 뭘까?
				// 버튼은 AddressBook에 있는데, 처리는 ModifyDialog에서 해야하니까!
				...
				ctrl.send(vo);
				if(ctrl.send(vo).getResult() == 1) {
					abook.refreshData();// 새로고침
				}
			}catch(Exception e){
				JOptionPane.showMessageDialog(this, "수정 중 에러가 발생했습니다." + e,
		 ...
		 */
			...
			try {
				...
				JOptionPane.showMessageDialog(this, "데이터 입력에 성공했습니다.","INFO", JOptionPane.INFORMATION_MESSAGE);		
				AddressCtrl ctrl = new AddressCtrl();
				...
				JOptionPane.showMessageDialog(this, "입력 중 에러가 발생했습니다." + e,
					"Error", JOptionPane.ERROR_MESSAGE);
			}			
		}
		setVisible(false);
