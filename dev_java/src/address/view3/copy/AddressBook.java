...
public class AddressBook extends JFrame {
	...
	private void detailActionPerformed() {
		...
		if(index.length == 0) {
			...
			JOptionPane.showMessageDialog(this, "데이터를 한 건만 선택하세요.", "Error", JOptionPane.ERROR_MESSAGE);
			return;
		} else {
			try {
				...
				JOptionPane.showMessageDialog(this, "오류가 발생했습니다. "+e.toString(),"Error", JOptionPane.ERROR_MESSAGE);
			}
		}
	}
	...
	private void addActionPerformed(ActionEvent evt) {
		...
		// 호출할 때 파라미터로는 사용자가 선택한 버튼의 이름, 화면보여줄까? , AddressVO(새로 입력하는 거니까!)
		// null, abook의 주소번지(원본-공유-static)
	...
	public void updateActionPerformed() {
		...
		if(index.length == 0) {
			...
			try {
				Integer id = Integer.parseInt(myTableModel.getValueAt(index[0], 0).toString());
				vo.setCommand("select");
//				System.out.println("1번 clear");
				vo.setId(id);
//				System.out.println("2번 clear");
				mDialog.set("수정", true, ctrl.send(vo), abook);
				mDialog.setVisible(true);
//				System.out.println("3번 clear");
				refreshData();
			} catch (Exception e) {
				JOptionPane.showMessageDialog(this, "오류가 발생했습니다. "+e.toString(),"Error", JOptionPane.ERROR_MESSAGE);
			}
	...
	private void deleteActionPerformed() {
		...
		if(index.length == 0) {
			...
			try {
				Integer id = Integer.parseInt(myTableModel.getValueAt(index[0], 0).toString());
				System.out.println("선택한 아이디: "+id);
				vo.setCommand("delete");
				vo.setId(id);
				ctrl.send(vo);
				refreshData();
			} catch (Exception e) {
				JOptionPane.showMessageDialog(this, "오류가 발생하였습니다. "+e.toString(),
										    	"Error", JOptionPane.ERROR_MESSAGE);
			}
		}		
