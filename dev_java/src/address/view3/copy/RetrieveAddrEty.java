...
public class RetrieveAddrEty {
	...
	public List<Map<String,Object>> myBatisRetrieve() {
		//MapperConfig.xml에서 오라클 서버 정보를 읽어서 커넥션 연결
		...
		// 호출 유무
		System.out.println("RetrieveAddrEty retrieve() 호출 성공");
		// 물리적으로 떨어져있는 오라클 서버의 접속을 위한 정보를 담음
		// 컴파일을 하지 않아도 된다. 그래서 버전관리 용이 - spring 프레임워크 - xml 모르는 - annotation
		String resource = "address/view3/copy/MapperConfig.xml";
		// IO클래스 통해 xml을 스캔 >> 커넥션, 연결통로
		Reader reader = null;
	    ...
	    try {
	    	...
	    	// I/O, network, JDBC API - 명시적으로 자원을 반납할 것을 권장하고 있음 - 자바튜닝팀
	    	reader.close();
