package test.kh0503;
// UML도구를 이용해서 클래스를 설계할 때 +:public, -:private, #:protected
class Circle {
	...
	public double radius; // double의 디폴트 값은 10=0.0
	// 파라미터 있는 생성자 선언 및 전변 radius 초기화 구현함
	// Circle c=new Circle()로는 double radius가 없는 생성자 호출은 불가함
	public Circle() {}
	public Circle(double radius) { // 파라미터가 있는 생성자가 하나라도 있으면 디폴트 생성자를 JVM이 제공하지 않는다.
		this.radius = radius;
	}
	...
	public void setRadius(double radius) { // radius에 3.1이 복사된다. call by value
		this.radius = radius;              // 파라미터로 복사된 3.1을 전역변수에 대입하시오.
	}
