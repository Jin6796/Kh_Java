...
// 메인에서 입력 혹은 수정, 혹은 상세보기라는 세 가지 업무에 대해서 Dialog_3 하나의 클래스를
// 단 한 번만 인스턴스화 한 자료를 가지고 모두 처리를 하고 싶다. 어떡하지?
// 입력할 때에는 빈 칸을 제공하고 새로 입력한 값을 저장해야 한다. - 업무 역할
// 수정일 때에는 기존에 등록된 정보에 대해서 일부를 변경지원한다.
...
public class Main_3 extends JFrame implements ActionListener {
	...
	public Main_3() {
		initDisplay(); // 생성자 안에서는 static이 없어서 인스턴스화. 세미콜론으로 끝났기 때문에 메소드 호출.
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (btnInsert == obj) {
			// 자손 클래스에서 부모클래스의 refreshData메소드를 호출하고 싶다
			// 그래서 this를 자손클래스의 set메소드에 파라미터로 넘겨주자
