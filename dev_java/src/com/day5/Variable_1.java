...
public class Variable_1 {
	...
	int age = 20; // 전역변수는 인스턴스화가 실행되고 나면(15번) 그 후 heap메모리 안에서 클래스 안에 초기화가 된다
	// 사용자 정의 메소드 선언 - 파라미터는 없어도 합법, 있어도 합법
	void methodA() {
		// 5번 라인의 전역변수의 값을 20에서 30으로 재정의함
		// 아래 코드에서 int를 붙이면 지역변수가 되고 int를 붙이지 않으면 전역변수에 저장된 값 20이 30으로 덮어쓰기 됨
		age = 30;
	}// end of methodA
	...
	public static void main(String[] args) {
		...
		// 아래코드에서는 30이 출력됨	
		System.out.println("after : "+v1.age);	// 10번 라인에 'int'를 붙이면 지역변수가 되어 20으로 출력
	}// end of main 
}////// end of Variable_1
